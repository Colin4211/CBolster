...
//
//  File:
//      CBolster\Applications\PlatformInfo\FloatEx.h
//
//  DocTool Settings:
//      <DT_SUB_TOPIC> .
...
{
    (void)(theFloat);
    typename CBBase::Float_Types<T_FLOAT>::float_info_type floatInfo = theFloat;

    printf( "%s -- sizeof -      float_type - %s\n",  floatInfo.float_type_name(), IntEx_IntToString(sizeof(typename CBBase::Float_Types<T_FLOAT>::float_type)).c_str() );
    printf( "%s -- sizeof - float_info_type - %s\n",  floatInfo.float_type_name(), IntEx_IntToString(sizeof(typename CBBase::Float_Types<T_FLOAT>::float_info_type)).c_str() );
